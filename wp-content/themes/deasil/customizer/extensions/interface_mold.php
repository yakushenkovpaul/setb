...
function customizer_library_register( $wp_customize ) {
	...
	foreach ( $options as $option ) {
		...
		if ( isset( $option['type'] ) ) {
			...
			customizer_library_selective_refresh($option, $wp_customize);
...
function customizer_library_selective_refresh($option, $wp_customize){
	$wp_customize->selective_refresh->add_partial( $option['id'], array(
    'selector' => '#'.$option['id'], // You can also select a css class
    ) );
}
